...
public final class OpenSourceInEditorAction extends Action {
    ...
    public void run() {
        ...
        if (object instanceof JavaSoftwareElement) {
            ...
            String message = "Java Software Element: " + softwareElement.getLabel();
            if (parentItem != null) {
                Object data = parentItem.getData();
                if (data instanceof Variant) {
                    String codeForVariationPoint = getCodeForVariationPoint(((Variant) data).getVariationPoint(),
                            (Variant) data);
                    message += TEXT_AVAILABLE_VARIANTS + codeForVariationPoint;
                }
            }
