...
public class SimilaritySwitchPredecessorAware extends ComposedSwitch<Boolean> {
	...
	private class ExpressionsSimilaritySwitch extends
		...
		public Boolean caseAssignmentExpression(AssignmentExpression exp1) {
			...
			Boolean childSimilarity = similarityChecker.isSimilar(child1,
					child2);
			...
			Boolean valueSimilarity = similarityChecker.isSimilar(value1,
					value2);
		...
		public Boolean caseEqualityExpression(EqualityExpression exp1) {
			...
			for (int i = 0; i < operators1.size(); i++) {
				Boolean childSimilarity = similarityChecker.isSimilar(
						operators1.get(i), operators2.get(i));
				if (childSimilarity == Boolean.FALSE) {
					return Boolean.FALSE;
				}
			}
			...
			for (int i = 0; i < children1.size(); i++) {
				Boolean childSimilarity = similarityChecker.isSimilar(
						children1.get(i), children2.get(i));
				if (childSimilarity == Boolean.FALSE) {
					return Boolean.FALSE;
				}
			}
	...
	private class StatementsSimilaritySwitch extends StatementsSwitch<Boolean> {
		...
		/**
		 * Flag if the position of a statement should be considered for
		 * similarity or not.
		 */
		 ...
		 *            Flag if the position of a statement should be considered
		 *            for similarity or not.
		 */
		 ...
		 *
		 * Only one throw statement can exist at the same code location. As a
		 * result the container similarity checked implicitly is enough for
		 * this.
		 *
		 ...
		 */
		private boolean differentPredecessor(Statement statement1,
		...
		/**
		 * Get the position of a statement in it's container. If the container
		 * is not a {@link StatementListContainer} the method will always return
		 * null.
		 *
		 * @param statement
	...
	@Override
	public Boolean defaultCase(EObject object) {
		// TODO: Check why proxies can occure here and if and how they must be
		// treated
