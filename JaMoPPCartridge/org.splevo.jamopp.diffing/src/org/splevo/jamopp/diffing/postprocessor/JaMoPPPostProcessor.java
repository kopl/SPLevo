 ...
 * <p>
 * After the diff process has been finished, too detailed differences are
 * detected and refined diffs for the next uper level of more coarse grained
 * diffs are created. For example, if a diff is for an expression, the parent
 * matches are walked up to find an appropriate AST element (e.g. a
 * FieldDeclaration). A {@link FieldChange} will be created and marked as
 * refined by the original diff.
 * </p>
 */
public class JaMoPPPostProcessor implements IPostProcessor {
	 ...
	 * <ol>
	 * <li>
	 * Scan the differences and build refined Diff elements for to detailed ones.
	 * (e.g. Expressions etc.)
	 * </li>
	 * <li>
	 * Remove nested diffs for statement changes.
	 * E.g. if an if statement has been changed, no additional (nested) diffs
	 * must be specified inside this.<br>
	 * If a more fine-grained level than statements is intended this needs to be adapted.
	 * </li>
	 * </ol>
	 *
	 * {@inheritDoc}
	...
	public void postDiff(Comparison comparison, Monitor monitor) {
		...
		for (Diff diff : comparison.getDifferences()) {
			if (diff.getMatch().getLeft() instanceof Statement
					|| diff.getMatch().getRight() instanceof Statement) {
