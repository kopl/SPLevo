 ...
 * 
 * The processor uses an internal switch based on the generated emf switch for
 * the modisco java model.
 */
public class UnmatchedElementProcessor {
	...
	/**
	 * Internal processor switch to handle the unmatched element according to
	 * the type of element the unmatch element is about.
	 */
	private class ProcessorSwitch extends JavaSwitch<DiffElement> {
		...
		/**
		 * Constructor requiring the UnmatchElement according to which the
		 * switchg should be done.
		 * 
		 * @param unmatchElement
		...
		public DiffElement caseImportDeclaration(ImportDeclaration element) {
			...
			if (unmatchElement.getSide() == Side.LEFT) {
				...
				// add ImportDelete
				final ImportDelete importDelete = Java2KDMDiffFactory.eINSTANCE
						.createImportDelete();
				importDelete.setImportRight(element);
				EObject leftContainer = matchManager.getMatchedEObject(element
						.getOriginalCompilationUnit());
				if (leftContainer != null) {
					importDelete
							.setLeftContainer((CompilationUnit) leftContainer);
				}
				return importDelete;
			}
		}
		 ...
		 */
		@Override
		public DiffElement caseClassDeclaration(
		...
		@Override
		public DiffElement caseFieldDeclaration(
		...
		@Override
		public DiffElement caseAbstractMethodDeclaration(
		...
		public DiffElement caseStatement(Statement statment) {
			...
			if (unmatchElement.getSide() == Side.LEFT) {
				return DiffElementFactory.createStatementInsert(statment);
			} else {
				return DiffElementFactory.createStatementDelete(statment, matchManager);
			}
		}
		 ...
		 */
		private Collection<DiffElement> buildSubDiffElements(
			...
			for (AbstractTypeDeclaration typeDeclaration : packageElement
					.getOwnedElements()) {
				if (typeDeclaration instanceof ClassDeclaration) {
					...
					logger.warn("unsupported package owned element: "
		...
		private PackageInsert createPackageInsert(Package packageElement) {
			final PackageInsert packageInsert = Java2KDMDiffFactory.eINSTANCE
					.createPackageInsert();
		...
		private PackageDelete createPackageDelete(Package packageElement) {
			PackageDelete packageDelete = Java2KDMDiffFactory.eINSTANCE
					.createPackageDelete();
			packageDelete.setPackageRight(packageElement);
			EObject leftContainer = matchManager
					.getMatchedEObject(packageElement.getPackage());
			if (leftContainer != null) {
		...
		private ClassInsert createClassInsert(ClassDeclaration classDeclaration) {
			final ClassInsert classInsert = Java2KDMDiffFactory.eINSTANCE
					.createClassInsert();
		...
		private ClassDelete createClassDelete(ClassDeclaration classDeclaration) {
			final ClassDelete classDelete = Java2KDMDiffFactory.eINSTANCE
					.createClassDelete();
			classDelete.setClassRight(classDeclaration);
			EObject leftContainer = matchManager
					.getMatchedEObject(classDeclaration.getPackage());
			if (leftContainer != null) {
		 ...
		 */
		private MethodInsert createMethodInsert(
				AbstractMethodDeclaration methodDeclaration) {
			MethodInsert methodInsert = Java2KDMDiffFactory.eINSTANCE
					.createMethodInsert();
		 ...
		 */
		private MethodDelete createMethodDelete(
				AbstractMethodDeclaration methodDeclaration) {
			MethodDelete methodDelete = Java2KDMDiffFactory.eINSTANCE
					.createMethodDelete();
			methodDelete.setMethodRight(methodDeclaration);

			if (methodDeclaration.getAbstractTypeDeclaration() != null) {
				EObject leftContainer = matchManager
						.getMatchedEObject(methodDeclaration
			...
			if (methodDelete.getLeftContainer() == null) {
				logger.warn("Missing left container for method declaration delete ("
						+ methodDeclaration + ")");
			}
			return methodDelete;
		}
		...
		private FieldInsert createFieldInsert(FieldDeclaration fieldDeclaration) {
			FieldInsert fieldInsert = Java2KDMDiffFactory.eINSTANCE
					.createFieldInsert();
		...
		private FieldDelete createFieldDelete(FieldDeclaration fieldDeclaration) {
			...
			if (fieldDelete.getLeftContainer() == null) {
				logger.warn("Missing left container for field declaration ("
						+ fieldDeclaration + ")");
			}
			return fieldDelete;
		}
