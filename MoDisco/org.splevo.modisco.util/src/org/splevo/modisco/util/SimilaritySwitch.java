 ...
 * 
 * Assumption: Both objects checked are not null.
 * 
 * 
 */
public class SimilaritySwitch extends JavaSwitch<Boolean> {
	 ...
	 */
	@Override
	public Boolean caseExpressionStatement(ExpressionStatement expStmt1) {

		ExpressionStatement expStmt2 = (ExpressionStatement) compareElement;

		Expression exp1 = expStmt1.getExpression();
		Expression exp2 = expStmt2.getExpression();

		Boolean expSimilarity = similarityChecker.isSimilar(exp1, exp2);
		if (expSimilarity != null && expSimilarity == Boolean.FALSE) {
			return expSimilarity;
		}
	...
	public Boolean caseImportDeclaration(ImportDeclaration importStatement) {
		...
		NamedElement imported1 = importStatement.getImportedElement();
		NamedElement imported2 = ((ImportDeclaration) compareElement)
				.getImportedElement();

		if (imported1 instanceof AbstractTypeDeclaration
				&& imported2 instanceof AbstractTypeDeclaration) {

			AbstractTypeDeclaration atd1 = (AbstractTypeDeclaration) imported1;
			AbstractTypeDeclaration atd2 = (AbstractTypeDeclaration) imported2;

			return similarityChecker.isSimilar(atd1, atd2);
		}

		if (imported1 instanceof Package && imported2 instanceof Package) {

			Package atd1 = (Package) imported1;
			Package atd2 = (Package) imported2;

			return checkPackageSimilarity(atd1, atd2);
		}

		if (imported1 instanceof MethodDeclaration
				&& imported2 instanceof MethodDeclaration) {

			MethodDeclaration method1 = (MethodDeclaration) imported1;
			MethodDeclaration method2 = (MethodDeclaration) imported2;

			return similarityChecker.isSimilar(method1, method2);
		}

		logger.warn("Unhandled import type detected: " + imported1);
		return null;
	}
	...
	public Boolean caseArrayType(ArrayType arrayType) {
		...
		return similarityChecker.isSimilar(arrayType1.getElementType()
				.getType(), arrayType2.getElementType().getType());
	}
	...
	public Boolean caseWildCardType(WildCardType wildCardType1) {
		...
		if (wildCardType1.getName() != null
				&& !wildCardType1.getName().equals(wildCardType2.getName())) {
			return Boolean.FALSE;
		}

		return similarityChecker.isSimilar(wildCardType1.getBound(),
				wildCardType2.getBound());
	}
	 ...
	 */
	@Override
	public Boolean caseVariableDeclaration(VariableDeclaration varDec1) {

		VariableDeclaration varDecl2 = (VariableDeclaration) compareElement;

		// check the variables name equality
		if (!varDec1.getName().equals(varDecl2.getName())) {
			return Boolean.FALSE;
		}
	 ...
	 * 
	 * @param method1
	 *            The abstract method declaration to compare with the compare
	 *            element.
	 * @return True/False if the abstract method declarations are similar or
	 *         not.
	 */
	@Override
	public Boolean caseAbstractMethodDeclaration(
			AbstractMethodDeclaration method1) {
	...
	public Boolean caseClassInstanceCreation(ClassInstanceCreation cic1) {
		...
		Boolean typeSimilarity = similarityChecker.isSimilar(cic1.getType()
				.getType(), cic2.getType().getType());
	...
	public Boolean casePackageAccess(PackageAccess packageAccess1) {
		...
		Boolean packageSimilarity = checkPackageSimilarity(
				packageAccess2.getPackage(), packageAccess1.getPackage());
		if (packageSimilarity != null && packageSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}

		Boolean compUnitSimilarity = similarityChecker.isSimilar(
				packageAccess1.getOriginalCompilationUnit(),
	 ...
	 *            The single variable declaration to compare with the compare
	 *            element.
	 * @return True/False if the single variable declarations are similar or
	 *         not.
	 */
	@Override
	public Boolean caseSingleVariableDeclaration(
			SingleVariableDeclaration varDeclaration1) {

		SingleVariableDeclaration varDeclaration2 = (SingleVariableDeclaration) compareElement;

		// Check name similarity
		if (varDeclaration1.getName() != null
				&& !varDeclaration1.getName().equals(varDeclaration2.getName())) {
			return Boolean.FALSE;
		}
	...
	@Override
	public Boolean caseVariableDeclarationStatement(
			VariableDeclarationStatement varDeclStatement1) {

		VariableDeclarationStatement varDeclStatement2 = (VariableDeclarationStatement) compareElement;

		// check fragment count
		if (varDeclStatement1.getFragments().size() != varDeclStatement2
				.getFragments().size()) {
			return Boolean.FALSE;
		}
		...
		for (int i = 0; i < varDeclStatement1.getFragments().size(); i++) {
			VariableDeclarationFragment frag1 = varDeclStatement1
					.getFragments().get(i);
			VariableDeclarationFragment frag2 = varDeclStatement2
					.getFragments().get(i);
			if (frag1.getName() != null
					&& !frag1.getName().equals(frag2.getName())) {
				return Boolean.FALSE;
	 ...
	 * </ul>
	 * 
	 * The then and else statements are not checked as part of the if statement
	 * check because this is only about the container if statement similarity.
	 * The contained statements are checked in a separate step of the compare
	 * process if the enclosing if statement matches.
	 * 
	 * @param ifStatement1
	...
	public Boolean caseIfStatement(IfStatement ifStatement1) {
		...
		Expression exp1 = ifStatement1.getExpression();
		Expression exp2 = ifStatement2.getExpression();
		Boolean expSimilarity = similarityChecker.isSimilar(exp1, exp2);
		if (expSimilarity != null && expSimilarity == Boolean.FALSE) {
			return expSimilarity;
		}
	...
	/**
	 * Check if two switch statements are similar.
	 * 
	 * Similarity is checked by:
	 * <ul>
	 * <li>similarity of the expressions</li>
	 * </ul>
	 * 
	 * @param switchStatement1
	 *            The statement to compare with the compare element.
	 * @return True/False whether they are similar or not.
	 */
	@Override
	public Boolean caseSwitchStatement(SwitchStatement switchStatement1) {

		SwitchStatement switchStatement2 = (SwitchStatement) compareElement;

		Expression exp1 = switchStatement1.getExpression();
		Expression exp2 = switchStatement2.getExpression();
		Boolean expSimilarity = similarityChecker.isSimilar(exp1, exp2);
		if (expSimilarity != null && expSimilarity == Boolean.FALSE) {
			return expSimilarity;
		}
	...
	/**
	 * Check if two switch case statements are similar.
	 * 
	 * Similarity is checked by:
	 * <ul>
	 * <li>similarity of the expressions</li>
	 * </ul>
	 * 
	 * @param switchCaseStatement1
	 *            The statement to compare with the compare element.
	 * @return True/False whether they are similar or not.
	 */
	@Override
	public Boolean caseSwitchCase(SwitchCase switchCaseStatement1) {

		SwitchCase switchCaseStatement2 = (SwitchCase) compareElement;

		Expression exp1 = switchCaseStatement1.getExpression();
		Expression exp2 = switchCaseStatement2.getExpression();
		Boolean expSimilarity = similarityChecker.isSimilar(exp1, exp2);
		if (expSimilarity != null && expSimilarity == Boolean.FALSE) {
			return expSimilarity;
		}
	 ...
	 * 
	 * The body is not checked as part of the for statement check because this
	 * is only about the container for statement similarity. The contained
	 * statements are checked in a separate step of the compare process if the
	 * enclosing for statement matches.
	 * 
	 * @param forStmt1
	 *            The statement to compare with the compare element.
	 * @return True/False whether they are similar or not.
	 */
	@Override
	public Boolean caseForStatement(ForStatement forStmt1) {

		ForStatement forStmt2 = (ForStatement) compareElement;

		if (forStmt1.getInitializers().size() != forStmt2.getInitializers()
				.size()) {
			return Boolean.FALSE;
		}

		if (forStmt1.getUpdaters().size() != forStmt2.getUpdaters().size()) {
			return Boolean.FALSE;
		}

		Boolean expSimilarity = similarityChecker.isSimilar(
				forStmt1.getExpression(), forStmt2.getExpression());
		if (expSimilarity != null && expSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}
	 ...
	 */
	@Override
	public Boolean caseEnhancedForStatement(EnhancedForStatement statement1) {

		EnhancedForStatement statement2 = (EnhancedForStatement) compareElement;
		...
		SingleVariableDeclaration var1 = statement1.getParameter();
		SingleVariableDeclaration var2 = statement2.getParameter();
		Boolean variableSimilarity = similarityChecker.isSimilar(var1, var2);
		if (variableSimilarity != null && variableSimilarity == Boolean.FALSE) {
			return variableSimilarity;
		}
	 ...
	 */
	@Override
	public Boolean caseWhileStatement(WhileStatement stmt1) {

		WhileStatement stmt2 = (WhileStatement) compareElement;
	...
	public Boolean caseCatchClause(CatchClause cc1) {
		...
		Boolean exceptionTypeSimilarity = similarityChecker.isSimilar(varDecl1
				.getType().getType(), varDecl2.getType().getType());
		if (exceptionTypeSimilarity != null
				&& exceptionTypeSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}
	...
	public Boolean caseInfixExpression(InfixExpression exp1) {
		...
		Boolean leftOpSimilarity = similarityChecker.isSimilar(
				exp2.getLeftOperand(), exp1.getLeftOperand());
		if (leftOpSimilarity != Boolean.TRUE) {
			return Boolean.FALSE;
		}

		Boolean rightOpSimilarity = similarityChecker.isSimilar(
				exp2.getRightOperand(), exp1.getRightOperand());
	 ...
	 * 
	 * @param exp1
	 *            The instance of expression to compare with the compare
	 *            element.
	...
	public Boolean caseAssignment(Assignment ass1) {
		...
		Boolean leftSimilarity = similarityChecker.isSimilar(
				ass1.getLeftHandSide(), ass2.getLeftHandSide());
		if (leftSimilarity != Boolean.TRUE) {
			return Boolean.FALSE;
		}

		Boolean rightSimilarity = similarityChecker.isSimilar(
				ass1.getRightHandSide(), ass2.getRightHandSide());
	 ...
	 * 
	 * @param exp1
	 *            The parenthesized expression to compare with the compare
	 *            element.
	...
	public Boolean caseParenthesizedExpression(ParenthesizedExpression exp1) {
		...
		return similarityChecker.isSimilar(exp1.getExpression(),
				exp2.getExpression());
	}
	...
	public Boolean caseTypeLiteral(TypeLiteral literal1) {
		...
		Boolean typeSimilarity = similarityChecker.isSimilar(
				literal1.getType(), literal2.getType());
		if (typeSimilarity == null && typeSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}
		return Boolean.TRUE;
	 ...
	 * 
	 * @param varAccess1
	 *            The single variable access to compare with the compare
	 *            element.
	 ...
	 * 
	 * @param varDeclFrag
	 *            The variable declaration fragment to compare with the compare
	 *            element.
	 * @return True/False if the variable declaration fragments are similar or
	 *         not.
	 */
	@Override
	public Boolean caseVariableDeclarationFragment(
		...
		// name check
		if (varDeclFrag.getName() != null
				&& !varDeclFrag.getName().equals(varDeclFrag2.getName())) {
			return Boolean.FALSE;
		}

		// container check
		Boolean containerSimilarity = similarityChecker.isSimilar(
				varDeclFrag.getVariablesContainer(),
				varDeclFrag2.getVariablesContainer());
		if (containerSimilarity != null) {
			return containerSimilarity;
		}

		logger.warn("variable declaration fragment container not supported: "
				+ varDeclFrag.getVariablesContainer());
	 ...
	 * 
	 * @param varDeclExp1
	 *            The variable declaration expression to compare with the
	 *            compare element.
	 * @return True/False if the variable declaration expressions are similar or
	 *         not.
	 */
	@Override
	public Boolean caseVariableDeclarationExpression(
			VariableDeclarationExpression varDeclExp1) {

		VariableDeclarationExpression varDeclExp2 = (VariableDeclarationExpression) compareElement;

		if (varDeclExp1.getFragments().size() != varDeclExp2.getFragments()
				.size()) {
			return Boolean.FALSE;
		}

		for (int i = 0; i < varDeclExp1.getFragments().size(); i++) {
			VariableDeclarationFragment varDeclFrag1 = varDeclExp1
					.getFragments().get(i);
			VariableDeclarationFragment varDeclFrag2 = varDeclExp2
					.getFragments().get(i);

			if (varDeclFrag1.getName() != null
					&& !varDeclFrag1.getName().equals(varDeclFrag2.getName())) {
				return Boolean.FALSE;
			} else if (varDeclFrag1.getName() == null
	...
	public Boolean caseFieldDeclaration(FieldDeclaration fDecl) {
		...
		for (int i = 0; i < fDecl.getFragments().size(); i++) {
			...
			VariableDeclarationFragment fragment2 = fDecl2.getFragments()
					.get(i);
		...
		if (fDecl.getType() != null && fDecl2.getType() != null) {
			Boolean typeSimilarity = similarityChecker.isSimilar(fDecl
					.getType().getType(), fDecl2.getType().getType());
			if (typeSimilarity != null && typeSimilarity == Boolean.FALSE) {
				return Boolean.FALSE;
			}
	...
	public Boolean caseAbstractTypeDeclaration(AbstractTypeDeclaration type1) {
		...
		if ((package1 == null && package2 != null)
				|| (package1 != null && package2 == null)) {
			return Boolean.FALSE;

			// if none is null check the package names
		} else if (package1 != null
				&& checkPackageSimilarity(package1, package2)) {
			return Boolean.TRUE;

			// if both packages are null check if both types are
			// inner class of the same enclosing class
		} else if (package1 == null
				&& type1.eContainer() instanceof AbstractTypeDeclaration
				&& type2.eContainer() instanceof AbstractTypeDeclaration) {
			AbstractTypeDeclaration enclosingAtd1 = (AbstractTypeDeclaration) type1
					.eContainer();
			AbstractTypeDeclaration enclosingAtd2 = (AbstractTypeDeclaration) type2
					.eContainer();
	 ...
	 * @return True/False if the anonymous class declarations are similar or
	 *         not.
	 */
	@Override
	public Boolean caseAnonymousClassDeclaration(
			AnonymousClassDeclaration aClassDecl1) {
	...
	public Boolean caseAnnotation(Annotation annotation1) {
		...
		for (int i = 0; i < annotation1.getValues().size(); i++) {
			AnnotationMemberValuePair pair1 = annotation1.getValues().get(i);
			AnnotationMemberValuePair pair2 = annotation2.getValues().get(i);

			Boolean valueSimilarity = similarityChecker.isSimilar(pair1, pair2);
	 ...
	 * 
	 * @param pair1
	 *            The annotation member value pair to compare with the compare
	 *            element.
	 * @return True/False if the annotation member value pairs are similar or
	 *         not.
	 */
	@Override
	public Boolean caseAnnotationMemberValuePair(AnnotationMemberValuePair pair1) {

		AnnotationMemberValuePair pair2 = (AnnotationMemberValuePair) compareElement;

		if (pair1.getName() != null && !pair1.getName().equals(pair2.getName())) {
			return Boolean.FALSE;
		}

		Boolean valueSimilarity = similarityChecker.isSimilar(pair1.getValue(),
				pair2.getValue());
		if (valueSimilarity != null && valueSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}
	 ...
	 * 
	 * Type parameters are always considered as similar. They might differ
	 * because of their type restriction (e.g. List<T extends ...>) but this is
	 * ignored for now.
	 * 
	 * @param param1
	...
	public Boolean caseArrayLengthAccess(ArrayLengthAccess access1) {
		...
		return similarityChecker.isSimilar(access1.getArray(),
				access2.getArray());
	}
	...
	public Boolean caseArrayAccess(ArrayAccess access1) {
		...
		Boolean arraySimilarity = similarityChecker.isSimilar(
				access1.getArray(), access2.getArray());
		if (arraySimilarity != null && arraySimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}

		Boolean indexSimilarity = similarityChecker.isSimilar(
				access1.getIndex(), access2.getIndex());
	 ...
	 * 
	 * @param ac1
	 *            The array creation expression to compare with the compare
	 *            element.
	 * @return True/False whether they are similar or not.
	 */
	@Override
	...
	public Boolean caseArrayInitializer(ArrayInitializer initializer1) {
		...
		if (initializer1.getExpressions().size() != initializer2
				.getExpressions().size()) {
			return Boolean.FALSE;
		}
	 ...
	 * 
	 * @param exp1
	 *            The conditional expression to compare with the compare
	 *            element.
	 * @return True/False whether they are similar or not.
	 */
	@Override
	public Boolean caseConditionalExpression(ConditionalExpression exp1) {
		...
		Boolean expSimilarity = similarityChecker.isSimilar(
				exp1.getExpression(), exp2.getExpression());
		if (expSimilarity != null && expSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}

		Boolean expElseSimilarity = similarityChecker.isSimilar(
				exp1.getElseExpression(), exp2.getElseExpression());
		if (expElseSimilarity != null && expElseSimilarity == Boolean.FALSE) {
			return Boolean.FALSE;
		}

		Boolean expThenSimilarity = similarityChecker.isSimilar(
				exp1.getThenExpression(), exp2.getThenExpression());
	 ...
	 * 
	 * Note: The type referenced by "this" might differ. But this is not checked
	 * at the moment.
	 * 
	 * @param thisExp
	...
	public Boolean casePrefixExpression(PrefixExpression exp1) {
		...
		Boolean operandSimilarity = similarityChecker.isSimilar(prefixExp1,
				prefixExp2);
	...
	/**
	 * Check if two initializers are similar.<br>
	 * This checks
	 * <ul>
	 * <li>the initializer name</li>
	 * </ul>
	 * 
	 * @param initializer1
	 ...
	 */
	@Override
	public Boolean caseInitializer(Initializer initializer1) {

		Initializer initializer2 = (Initializer) compareElement;

		String name1 = initializer1.getName();
		String name2 = initializer2.getName();
	...
	public Boolean caseMethodInvocation(MethodInvocation methodInvocation) {
		...
		Boolean methodSimilarity = similarityChecker
				.isSimilar(method1, method2);
		...
		for (int i = 0; i < methodInvocation.getArguments().size(); i++) {
			...
			Expression methodArgument2 = methodInvocation2.getArguments()
					.get(i);

			Boolean argumentSimilarity = similarityChecker.isSimilar(
					methodArgument1, methodArgument2);
			if (argumentSimilarity != null
					&& argumentSimilarity == Boolean.FALSE) {
				return Boolean.FALSE;
			}
		}
	 ...
	 * 
	 * @param sci1
	 *            The super constructor invocation to compare with the compare
	 *            element.
	 * @return True/False if the super constructor invocations are similar or
	 *         not.
	 */
	@Override
	public Boolean caseSuperConstructorInvocation(
			SuperConstructorInvocation sci1) {
		...
		for (int i = 0; i < sci1.getArguments().size(); i++) {
			Boolean argSimilarity = similarityChecker.isSimilar(sci1
					.getArguments().get(i), sci2.getArguments().get(i));
	 ...
	 * 
	 * @param smi1
	 *            The super method invocation to compare with the compare
	 *            element.
	...
	public Boolean caseSuperMethodInvocation(SuperMethodInvocation smi1) {
		...
		for (int i = 0; i < smi1.getArguments().size(); i++) {
			Boolean argSimilarity = similarityChecker.isSimilar(smi1
					.getArguments().get(i), smi2.getArguments().get(i));
			if (argSimilarity != null && argSimilarity == Boolean.FALSE) {
				return Boolean.FALSE;
			}
		}
	 ...
	 * 
	 * @param type1
	 *            The unresolved type declaration to compare with the compare
	 *            element.
	 * @return True/False if the unresolved type declarations are similar or
	 *         not.
	 */
	@Override
	 ...
	 * Blocks have no attributes to check and can be distinguished by their
	 * location only. However, container similarity is not expected to be
	 * checked.
	 * 
	 * @param block
	 ...
	 */
	@Override
	public Boolean caseTryStatement(TryStatement stmt) {
		return Boolean.TRUE;
	}
	...
	/**
	 * TagElement elements are always ignored and true is returned.
	 * {@inheritDoc}
	 */
	@Override
	 ...
	 */
	private Boolean checkPackageSimilarity(Package packageElement,
		...
		if (packageElement == referencePackage) {
			...
			String path1 = JavaModelUtil.buildPackagePath(packageElement);
			String path2 = JavaModelUtil.buildPackagePath(referencePackage);
