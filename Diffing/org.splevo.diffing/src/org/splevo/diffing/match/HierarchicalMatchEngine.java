...
/**
 * The default match engine does not take the hierarchical structure of the
 * model into account. As the location of model elements is essential for our
 * match strategies and moved elements are treated as separate ADD and DELETE,
 * this match engine implements a "level by level" process.
 * <p>
 * <b>Two-Way Comparison only</b><br>
 * This match engine does not support three-way matches. So the origin part of
 * the scop is ignored at all.
 * </p>
 */
public class HierarchicalMatchEngine implements IMatchEngine {
	...
	private static Logger logger = Logger
			.getLogger(HierarchicalMatchEngine.class);
	 ...
	 *            The equality helper to check equality and to be wired with the
	 *            comparison model.
	 * @param equalityStrategy
	 *            The equality strategy to use.
	 * @param ignoreStrategy
	 *            the strategy which elements must not be matched and can be
	 *            ignored.
	 */
	public HierarchicalMatchEngine(IEqualityHelper equalityHelper,
	...
	/**
	 * This methods will delegate to the proper "match(T, T, T)" implementation
	 * according to the types of {@code left}, {@code right} and {@code origin}.
	 *
	 * @param comparison
	 *            The comparison to which will be added detected matches.
	 * @param scope
	 *            The comparison scope that should be used by this engine to
	 *            determine the objects to match.
	 * @param left
	 ...
	 */
	protected void match(Comparison comparison, IComparisonScope scope,
			final Notifier left, final Notifier right, Monitor monitor) {
		if (left instanceof ResourceSet || right instanceof ResourceSet) {
			match(comparison, scope, (ResourceSet) left, (ResourceSet) right,
					monitor);
			...
			throw new IllegalArgumentException(
					"Unhandled type of elements to match. (" + left + ", "
							+ right + ")");
		}
	}

	/**
	 * This will be used to match the given {@link ResourceSet}s. This default
	 * implementation will query the comparison scope for these resource sets
	 * children, then delegate to an {@link IResourceMatcher} to determine the
	 * resource mappings.
	 *
	 * The content of the matched resources is then pushed into the resource
	 * specific match method.<br>
	 *
	 * <b>Note:</b><br>
	 * In contrast to the DefaultMatchEngine, the match for eObjects recursively
	 * iterates through sub elements of the models. As a result, only the root
	 * elements of the resources are matched and not the plain list of all all
	 * contained elements.
	 *
	 * @param comparison
	 *            The comparison to which will be added detected matches.
	 ...
	 */
	protected void match(Comparison comparison, IComparisonScope scope,
			ResourceSet left, ResourceSet right, Monitor monitor) {
		final Iterator<? extends Resource> leftChildren = scope
				.getCoveredResources(left);
		final Iterator<? extends Resource> rightChildren = scope
				.getCoveredResources(right);
		final Iterator<? extends Resource> originChildren = Iterators
				.emptyIterator();

		final IResourceMatcher resourceMatcher = createResourceMatcher();
		final Iterable<MatchResource> mappings = resourceMatcher
				.createMappings(leftChildren, rightChildren, originChildren);
	...
	/**
	 * Build the matches for two given resources. The direct sub elements will
	 * be detected and pushed into a match process for sub elements.
	 *
	 * @param comparison
	 ...
	 */
	protected void match(Comparison comparison, final Resource leftRes,
			final Resource rightRes, Monitor monitor) {
		...
		List<Match> matches = match(comparison, leftElements, rightElements,
				monitor);
		comparison.getMatches().addAll(matches);
	}

	/**
	 * Create matches for the provided elements and trigger a match process for
	 * the child elements in case of a match.
	 *
	 * @param comparison
	 ...
	 * @param orderAware
	 *            Flag if the elements should be matched order aware
	 * @return The list of created matches.
	 */
	private List<Match> match(Comparison comparison,
			List<EObject> leftElements, List<EObject> rightElements,
			Monitor monitor) {
		...
		for (EObject leftElement : leftElementsInScope) {
			...
			for (EObject rightElement : rightElementsInScope) {
				if (equalityStrategy.areEqual(leftElement, rightElement)) {
					...
					List<Match> subMatches = match(comparison,
							leftElement.eContents(), rightElement.eContents(),
							monitor);
	...
	/**
	 * Create match objects with only right references set for a list of new
	 * elements.
	 *
	 * @param elements
	...
	/**
	 * Get a filtered list of elements without any elements that are not in
	 * scope.
	 *
	 * @param elements
	 *            The list to filter.
	 * @return The filtered list. If the list is null, an empty list will be
	 *         returned
	 */
	...
	/**
	 * This will be used to create the resource matcher that will be used by
	 * this match engine.
	 *
	 * @return An {@link IResourceMatcher} that can be used to retrieve the
	 *         {@link MatchResource}s for this comparison.
	 */
	 ...
	 * The strategy can assume that the compared elements are in the same model
	 * hierarchy.
	 */
	public interface EqualityStrategy {

		/**
		 * Check if two elements are equal. They are located at a matching
		 * position. So container similarity can be assumed.
		 *
		 * @param left
	...
	/**
	 * A strategy function to check if an element should be ignored from
	 * matching.
	 */
