 ...
 *
 *****How does that work?*****************************************
 * As a first step, the analyzer extracts all relevant content
 * from a VPMGraph and stores that within a Lucene index. Through
 * storing additional informations about the indexed text, Lucene
 * provides the ability to extract vectors from given index 
 * content. By calculating the cosine similarity for all possible
 * links (cross product of all VPs), semantic dependencies will be
 * found. Those results can be displayed within the VPMGraph or
 * the Refinement Browser. 
 *****************************************************************
 *
 * @author Daniel Kojic
 *
...
public class SemanticVPMAnalyzer extends AbstractVPMAnalyzer{
	...
	/**
	 * This method uses the IndexASTNodeSwitch to extract the text from the given Node.
	 * It iterates through all child elemenzts.
	 * 
	 * @param id The ID used to store the text in the Lucene index.
	 * @param node The corresponding {@link Node}.
	 * @param vp The corresponding {@link VariationPoint}.
	 * @param indexComments Determines if comments should be indexed or ignored.
	...
	/**
	 * Gets the similarity. Use the configuration if specified, 
	 * otherwise use the default value stored in the Constants class.
	 * 
