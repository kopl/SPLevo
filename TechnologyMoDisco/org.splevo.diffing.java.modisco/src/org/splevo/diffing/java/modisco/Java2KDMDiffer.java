...
import org.splevo.diffing.java.modisco.diff.JavaModelDiffEngine;
import org.splevo.diffing.java.modisco.match.JavaModelMatchEngine;
import org.splevo.diffing.java.modisco.match.JavaModelMatchScopeProvider;
import org.splevo.diffing.java.modisco.postprocessor.DiffModelPostProcessor;
...
public class Java2KDMDiffer extends JavaDiffer {
	 ...
	 */
	@Override
	public DiffModel doDiff(URI leadingModelDirectory,
			URI integrationModelDirectory, Map<String, Object> diffingOptions)
			throws DiffingException {

		@SuppressWarnings("unchecked")
		List<String> ignorePackages = (List<String>) diffingOptions
		...
		try {
			matchModel = matchEngine.modelMatch(integrationModel, leadingModel,
					matchOptions);
		...
		DiffModelPostProcessor postProcessor = new DiffModelPostProcessor(
				javaModelDiffEngine.getMatchManager());
	...
	private File findRootModelFileInDirectory(final URI basePath) {
		...
		String[] rootFiles = leadingModelDirectory
				.list(new PatternFilenameFilter(".*java2kdm\\.xmi"));
		File moDiscoRootFile = new File(leadingModelDirectory.getAbsolutePath()
				+ File.separator + rootFiles[0]);
	...
	private Map<String, Object> buildMatchOptions(List<String> ignorePackages) {
		...
		JavaModelMatchScopeProvider scopeProvider = new JavaModelMatchScopeProvider(
				ignorePackages);
