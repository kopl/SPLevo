 ...
 * 
 */
public class FeatureMapperModelsBuilder implements
		FeatureModelBuilder<FeatureMapperModelSet> {

	private static Logger logger = Logger
			.getLogger(FeatureMapperModelsBuilder.class);
	 ...
	 */
	@Override
	public FeatureMapperModelSet build(VariationPointModel vpm,
		...
		for (VariationPointGroup variationPointGroup : vpm
			...
			for (VariationPoint variationPoint : variationPointGroup
				...
				for (Variant variant : variationPoint.getVariants()) {
					...
					for (SoftwareElement softwareElement : softwareEntities) {
						...
						Commentable jamoppElement = ((JaMoPPSoftwareElement) softwareElement)
								.getJamoppElement();
			...
			for (Resource resource : solutionModelResources) {
				if (resource != null && !resource.getContents().isEmpty()) {
					...
					SolutionModelRef solutionModelRef = mappingFactory
							.createSolutionModelRef();
					...
					logger.error("Could not create solution model reference for resource "
							+ resourceIdentifier);
				}
			}
	...
	public static void saveFeatureModel(FeatureModel featureModel, File filePath)
		...
		saveModelToPath(featureModel, filePath, "featureModel",
				featureModelFileExtension);
	}
	...
	private static void saveModelToPath(EObject model, File filePath,
		...
		final Resource resource = resSet.createResource(URI
				.createPlatformResourceURI(filePath.getPath() + File.separator
